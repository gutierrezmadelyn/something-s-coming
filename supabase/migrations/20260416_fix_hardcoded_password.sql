-- Migration: Fix hardcoded password security issue
-- Date: 2026-04-16
-- Description: Generate random secure passwords instead of using hardcoded default

-- ============================================
-- UPDATE admin_import_user FUNCTION
-- ============================================
-- Generate a random 16-character password for each new user
-- instead of using a hardcoded default password

CREATE OR REPLACE FUNCTION admin_import_user(
  p_email TEXT,
  p_name TEXT,
  p_password TEXT DEFAULT NULL,  -- Now NULL by default, will generate random if not provided
...
BEGIN
  ...
  -- Generate random password if not provided (16 chars alphanumeric + special)
  IF p_password IS NULL OR p_password = '' THEN
    v_actual_password := encode(gen_random_bytes(12), 'base64');
  ELSE
    v_actual_password := p_password;
  END IF;
  ...
  IF v_existing_auth_id IS NOT NULL THEN
    ...
    END IF;
  ELSE
    -- Create new auth user with secure password
    v_user_id := gen_random_uuid();
    v_encrypted_pw := crypt(v_actual_password, gen_salt('bf'));
...
COMMENT ON FUNCTION admin_import_user(TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT[], TEXT, TEXT, FLOAT, FLOAT) IS
'Imports a user with admin privileges. If no password is provided, generates a random secure password.
Users will need to use password reset to set their own password.';
