-- Fix: Drop the old admin_import_user function to resolve PostgREST overload ambiguity
-- Problem: Two versions of admin_import_user exist with different signatures
-- Old: (text, text, text, text, text, text, text[], text, text) - 9 params without p_password
-- New: (text, text, text, text, text, text, text, text[], text, text) - 10 params with p_password
-- PostgREST error: "Could not choose the best candidate function"

-- Drop ALL overloaded versions first
DROP FUNCTION IF EXISTS public.admin_import_user(text, text, text, text, text, text, text[], text, text);
DROP FUNCTION IF EXISTS public.admin_import_user(text, text, text, text, text, text, text, text[], text, text);
