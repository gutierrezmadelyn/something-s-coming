...
import { Avatar, Tag, Btn, Toggle } from "./ui";
import { Handshake, MessageCircle, Lock, Pencil } from "lucide-react";
...
export default function MyProfile({ profile, privacySettings, onPrivacyChange, matches, onEdit }) {
  ...
  const sections = [
    ...
    { label: "Quiere aprender", items: Array.isArray(profile.wantsToLearn) ? profile.wantsToLearn : (profile.wantsToLearn ? [profile.wantsToLearn] : []), bg: S.yellowBg, color: S.yellowText },
  ...
  const stats = [
    { icon: Handshake, value: matches?.length || profile.matchCount || 0, label: "Matches", color: S.purple },
    { icon: MessageCircle, value: profile.conversationsStarted || 0, label: "Chats", color: S.blue },
  ];
  ...
  return (
    <div style={{ padding: "16px 0" }}>
      ...
      <div style={{ background: S.card, borderRadius: "18px", padding: "18px", border: `1px solid ${S.border}`, marginBottom: "12px" }}>
        <h3 style={{ fontSize: "15px", color: S.text, margin: "0 0 8px", fontWeight: 700, display: "flex", alignItems: "center", gap: "6px", fontFamily: "'DM Sans', sans-serif" }}><Lock size={14} color={S.textSec}/> Privacidad</h3>
      ...
      <div style={{ background: S.card, borderRadius: "18px", padding: "18px", border: `1px solid ${S.border}` }}>
        ...
        <Btn variant="outline" style={{ width: "100%", marginTop: "12px" }} onClick={onEdit}><Pencil size={14}/> Editar mi perfil</Btn>
      </div>
    </div>
  );
}
